...
{
	// Draw flashlights  with the main geometry only
...
void CClientShadowMgr::ComputeShadowDepthTextures( const CViewSetup &viewSetup )
{
	...
	for ( int j = 0; j < nActiveDepthShadowCount; ++j )
	{
		...
		if ( !bGotShadowDepthTexture )
		{
			...
			static int shadowMaps = 0;
			if( shadowMaps < 10 ) // Original variable name was unclear on what this is exactly. Guessing is shadow maps number.
			{
