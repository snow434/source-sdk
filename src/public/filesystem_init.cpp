...
static FSReturnCode_t TryLocateGameInfoFile( char *pOutDir, int outDirLen, bool bBubbleDir )
{
	...
	do
	{
		if (DoesFileExistIn(pOutDir, GAMEINFO_FILENAME) || DoesFileExistIn(pOutDir, GAMEINFO_FILENAME_ALTERNATE))
		{
			return FS_OK;
		}
	} 
...
FSReturnCode_t LocateGameInfoFile( const CFSSteamSetupInfo &fsInfo, char *pOutDir, int outDirLen )
{
	...
	if ( pProject )
	{
		if (DoesFileExistIn(pProject, GAMEINFO_FILENAME) || DoesFileExistIn(pOutDir, GAMEINFO_FILENAME_ALTERNATE))
		{
